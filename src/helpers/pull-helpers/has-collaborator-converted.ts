...
export async function hasCollaboratorConvertedPr(
  context: Context<"pull_request.opened" | "pull_request.ready_for_review" | "pull_request.converted_to_draft">
) {
  ...
  const timeline = await context.octokit.issues.listEvents({
    owner: owner.login,
    repo: name,
    issue_number: number,
  });
